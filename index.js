...
Bitmap.prototype.setbit = function(key, offset, val, next) {
  ...
  function callback(err, resp) {
    ...
    return next(null, +resp.toString())
  }
...
Bitmap.prototype.bitcount = function() {
  ...
  if (args.length < 3 && !this._isAggr) {
    this.client.BITCOUNT.apply(this.client, args)
    return this
  }
...
Aggregate.prototype.exec = function(next) {
  ...
  if (dest && cmd) {
    ...
    callback = function(err, resp) {
      if (err) return next(err)
      if (resp) {
        resp = resp[resp.length - (clean ? 2 : 1)]
        resp = !isGet ? +resp.toString() : new BitArray(resp)
      }
